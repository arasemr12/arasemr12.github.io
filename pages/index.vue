<template>
  <div v-if="json" class="w-full h-full">
    ...
    <div class="w-hull h-full flex flex-col items-center">
      <div class="w-full lg:w-2/3 flex flex-col items-center gap-5" id="posts">
        <h1 class="text-xl font-bold uppercase">My posts</h1>
        <div class="w-full border-2 border-gray-800 py-6 px-6 rounded-lg flex flex-col gap-4" v-for="post in posts" :key="post._id">
          <div class="flex flex-col w-full border-2 border-gray-800 py-4 px-4 rounded-full">
            <span class="text-gray-400">{{post.author.email}}</span>
            <span class="text-xl">{{post.author.username}}</span>
          </div>
          <span class="break-words whitespace-pre-wrap">{{post.content}}</span>
          <a :href="`https://upss.ga/posts/${post._id}`" target="_blank" class="text-blue-600 underline">read more</a>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    async update(){
      try {
        ...
        let postsres = await fetch("/api/posts");
